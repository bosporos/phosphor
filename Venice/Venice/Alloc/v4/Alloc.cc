...
void Block::hook_informs_block_of_deallocation_request (void * object_to_deallocate)
{
    ...
    // sub_fetch instead of fetch_sub (faster)
    __atomic_sub_fetch (&this->_allocation_count, 1, __ATOMIC_ACQ_REL);

    if (__atomic_load_n (&this->_is_head, __ATOMIC_ACQUIRE) == true) {
        // nothing
    } else {
        // make sure that empty-enough check only activates if the empty check fails
        ...
        } else if (this->allocation_count.fetch_sub (1, Ordering::AcqRel) <= __atomic_load_n (&this->active_linkage, __ATOMIC_ACQUIRE)->metric->metric_block_sufficient_emptiness (this)) {
...
void ActiveLocalLinkage::hook_block_informs_linkage_of_empty_block (Block * informer)
{
    ...
    // 26.1.2020:
    // if we're head:
    // we're allocation-locked, so allocation requests will try to blow up
    // solution: __shadow_flags variable on the Block
