...
BlockLinkage::BlockLinkage (Metric * metric)
    : metric { metric }
{}
...
BlockAllocationLinkage::~BlockAllocationLinkage ()
{
    ...
    if (this->head->inner->allocation_count.load (Ordering::Acquire) == 0) {
        // just send it to the sized heap-- those are okay with having empties
        // @TODO: find a way to get rid of unsized->sized recycling
        //  - putting a sibling_unsized method on RSRH?
        __atomic_load_n (&this->parent, __ATOMIC_ACQUIRE)->parent_sized->hook_child_recycling_block (this->head->inner);
...
void * LocalAllocatingHeap::allocate (math::_u16 const obj_size)
{
...
/*
 * BlockReclamationLinkage
 */

UnsizedBlockReclamationLinkage::UnsizedBlockReclamationLinkage (Metric * metric)
    : BlockLinkage (metric)
    , parent { nullptr }
    , head { nullptr }
    , length (0)
...
UnsizedBlockReclamationLinkage::~UnsizedBlockReclamationLinkage ()
{
    this->rl_lock.lock ();
    this->length.store (0, Ordering::Release);
    // gotta free to the parent
    internal::DLListItem<Block> *li = this->head, *off;
    while (li != nullptr) {
        this->parent->parent_unsized->hook_block_is_empty (li->inner);
        off = li->right;
        internal::free_dllistitem (li);
        li = off;
    }
...
void UnsizedBlockReclamationLinkage::hook_block_is_empty (Block * block)
{
    this->rl_lock.lock ();
    if (this->head != nullptr) {
        if (this->length.load (Ordering::Acquire) > this->metric->metric_unsized_block_reclamation_linkage_max_length (this)) {
            this->rl_lock.unlock ();
            // pass it up if we're over quota
            this->parent->hook_block_is_empty (block);
        } else {
            this->head->add_left (internal::alloc_dllistitem (block));
            this->head = this->head->left;
            this->length.fetch_add (1, Ordering::AcqRel);
        }
    } else {
        this->head = internal::alloc_dllistitem (block);
        this->length.fetch_add (1, Ordering::AcqRel);
    }
    __atomic_store_n (&block->parent, this, __ATOMIC_RELEASE);
    this->rl_lock.unlock ();
...
void UnsizedBlockReclamationLinkage::hook_block_is_empty_enough (Block * block)
{
    // we're unsized, so: impossible
    abort ();
}

Block * UnsizedBlockReclamationLinkage::hook_child_requesting_block ()
{
    this->rl_lock.lock ();
    if (this->head != nullptr) {
        this->length.fetch_sub (1, Ordering::AcqRel);
        internal::DLListItem<Block> * li = this->head->right;
        Block * block                    = this->head->inner;
        internal::free_dllistitem (this->head);
        this->head = li;
        li->left   = nullptr;
        this->rl_lock.unlock ();
...
SizedBlockReclamationLinkage::SizedBlockReclamationLinkage (Metric * metric, math::_u16 const obj_sz)
    : BlockLinkage (metric)
    , rl_lock ()
    , parent { nullptr }
    , head { nullptr }
...
SizedBlockReclamationLinkage::~SizedBlockReclamationLinkage ()
{
    this->rl_lock.lock ();
    internal::DLListItem<Block> *li = head->left, *new_li;
    while (li != nullptr) {
...
void SizedBlockReclamationLinkage::hook_block_is_empty (Block * block)
{
    this->rl_lock.lock ();
    __atomic_load_n (&this->parent, __ATOMIC_ACQUIRE)->parent_unsized->hook_block_is_empty (block);
    this->rl_lock.unlock ();
}
