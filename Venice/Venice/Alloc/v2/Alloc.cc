...
Block * SizedBlockChain::needs_block ()
{
    // Swap out head
    this->chain_lock.lock ();
    if (this->right_length.load (Ordering::Acquire) > 0) {
        // Swap from right
        this->left_length.fetch_add (1, Ordering::AcqRel);
        this->right_length.fetch_sub (1, Ordering::AcqRel);

        DLListItem<Block> * right = this->head.right;
        if (this->head.left != nullptr)
            this->head.left->right = right;
        right->left = this->head.left;
        if (right->right != nullptr)
            right->right->left = &this->head;
        this->head.right = right->right;
        right->right     = &this->head;

        Block * tmp      = right->inner;
        right->inner     = this->head.inner;
        this->head.inner = tmp;

        this->chain_lock.unlock ();
        return this->head.inner;
    } else {
        this->chain_lock.unlock ();
...
const static int ___vnz_chain_sizes[] {
    1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48, 64, 96, 128, 192, 256, 384, 512, 768, 1024
};

extern "C"
{
    _i64 __vnz_chain_lookup (_u64 size);
}
...
{
    for (_i32 i = 0; i < 20; i++) {
        new (&this->chains[i]) SizedBlockChain (___vnz_chain_sizes[i], this);
    }
}

LSHeap::~LSHeap ()
{
    // although chains is owned, since it's union-ized, there's no language
    // guarantee that it was initialized, so we have to manually destruct it`
    for (_i32 i = 0; i < 20; i++) {
        this->chains[i].~SizedBlockChain ();
    }
    // Not that the destructor does anything at the moment, but eh
}

void * LSHeap::alloc_object (_u64 size)
{
    _u64 chain_index = __vnz_chain_lookup (size);
    if (chain_index < 20) {
        return this->chains[chain_index].alloc_object ();
    } else {
        return malloc (size);
    }
}

Block * LSHeap::request_block ()
{
    // SizedBlockChain can call from needs_block() when it needs an empty
    Block * block = this->first_sized_ancestor->request_block_no_propagation ();
    if (block == nullptr) {
        block = this->first_unsized_ancestor->request_block_no_propagation ();
    }
    if (block == nullptr) {
        block = this->first_sized_ancestor->request_block ();
    }
    return block;
}

void LSHeap::handle_free_block (Block * block)
{
    // called by chains when a block is freed
    if (block->objects_free.load (Ordering::Acquire) == block->object_capacity) {
        this->first_unsized_ancestor->handle_free_block (block);
    } else {
        this->reclaim_filled_block (block);
    }
}

void LSHeap::reclaim_filled_block (Block * block)
{
    // called by chains when a partially full block is freed
    this->first_sized_ancestor->reclaim_filled_block (block);
}
